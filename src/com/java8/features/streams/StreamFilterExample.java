...
public class StreamFilterExample {
	...
	public static void main(String[] args) {
		...
		/*
		 * names.stream().filter(s -> s.length() >
		 * 3).collect(Collectors.toList()).forEach(System.out::println);
		 */
		
		/*
		 * PersonRepository.getAllPersons() .stream() .filter(per -> per.getHeight() >=
		 * 140) .collect(Collectors.toList()) .forEach(System.out::println);
		 */
